...
class OrderControl(APIView):
    ...
    def patch(self, request, id=None):
        ...
        if serializer.is_valid():
            ...
            # if updated_order.status == 'delivered':
            #     try:
            #         # Retrieve the product and stock based on the product_ID from the order
            #         product = Product.objects.get(id=updated_order.product_ID)
            #         stock = Stock.objects.filter(supplier_email=requesting_user.email).first()
                    
            #         if stock and stock.quantity >= updated_order.quantity:
            #             # Reduce the stock quantity by the order quantity
            #             stock.quantity -= updated_order.quantity
            #             stock.save()
            #         else:
            #             return Response({"status": "error", "message": "Insufficient stock"}, status=400)
